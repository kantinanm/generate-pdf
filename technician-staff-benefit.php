...
foreach ($obj[0] as $key => $row) {
	...
	$dataEngineer[$index_st]=array("name"=>$row->firstname,"nu_net"=>$row->staff_nu_net,"rotage"=>$row->rotage,"preset_x"=>$row->preset_x,"preset_y"=>$row->preset_y,"blank"=>$row->blank,"font_size"=>$row->font_size);
    //echo $key;
	$index_st++;
}
