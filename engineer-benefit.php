...
$url = 'http://localhost:8000/api/engineers-monthly';
...
foreach ($obj[0] as $key => $row) {
	...
	$dataEngineer[$index_st]=array("name"=>$row->firstname,"nu_net"=>$row->engineer_nu_net,"rotage"=>$row->rotage,"preset_x"=>$row->preset_x,"preset_y"=>$row->preset_y,"blank"=>$row->blank,"font_size"=>$row->font_size);
    //echo $key;
	$index_st++;
	if($index_st==11){
		break;
...
$dataEngineer[11]=array("name"=>"กรกฎ","nu_net"=>"korakodn","rotage"=>90,"preset_x"=>4,"preset_y"=>9,"blank"=>"yes","font_size"=>11);
